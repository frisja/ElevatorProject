...
// it on `port`
func Bcast_transmitter(port int, chans ...interface{}) {
	Bcast_check_args(chans...)
	...
	conn := conn.Dial_broadcast_UDP(port)
...
func Bcast_receiver(port int, chans ...interface{}) {
	Bcast_check_args(chans...)
...
func Bcast_check_args(chans ...interface{}) {
	n := 0
	for range chans {
		n++
	}
