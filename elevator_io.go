...
func Elevator_io_poll_buttons(receiver chan<- ButtonEvent) {
...
func Elevator_io_poll_floor_sensor(receiver chan<- int) {
	prev := -1
	for {
		time.Sleep(_pollRate)
		v := GetFloor()
...
func Elevator_io_poll_stop_button(receiver chan<- bool) {
	prev := false
	for {
		time.Sleep(_pollRate)
		v := GetStop()
...
func Elevator_io_poll_obstruction_switch(receiver chan<- bool) {
	prev := false
	for {
		time.Sleep(_pollRate)
		v := GetObstruction()
